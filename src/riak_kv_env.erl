...
-type erts_key()    :: atom().
-type erts_param()  :: {erts_desc(), erts_key(), erts_value()}.
-type erts_params() :: list(erts_param()).
-type erts_value()  :: non_neg_integer().
-type exe_arg()     :: binary().
...
%% TODO: Figure out recommended ?ERTS_GCGEN_* range

-define(ERTS_DBUF_BUSY,       ?M(32)).  % erl +zdbbl x 1024
-define(ERTS_ETS_MIN,         ?K(16)).  % erl +e, named ETS tables
-define(ERTS_GCGEN_MAX,        ?K(4)).  % erl -env ERL_FULLSWEEP_AFTER
-define(ERTS_GCGEN_MIN,           16).  % erl -env ERL_FULLSWEEP_AFTER
-define(ERTS_PORT_MIN,        ?K(64)).  % erl +Q, power of 2
-define(ERTS_PROC_MIN,       ?K(512)).  % erl +P, power of 2
-define(ERTS_SCHED_PCT,           95).  % Percentage schedulers/cores
-define(ERTS_THRD_MIN,             8).  % erl +A, mostly irrelevant
-define(OS_CORE_MIN,         ?K(512)).  % In blocks, generally 512 or 1024 bytes
-define(OS_FILE_MIN,          ?K(32)).  % Minimum open files

%% These will be logged in reverse order.
-define(ERTS_LIMIT_PARAMS, [
    {"ERTS dist buffer busy",   dist_buf_busy_limit,    ?ERTS_DBUF_BUSY},
    {"ERTS thread pool",        thread_pool_size,       ?ERTS_THRD_MIN},
    {"ERTS ETS table",          ets_limit,              ?ERTS_ETS_MIN},
    {"ERTS ports",              port_limit,             ?ERTS_PORT_MIN},
    {"ERTS process",            process_limit,          ?ERTS_PROC_MIN}
]).
...
-spec erts_limits(Params :: erts_params(), Result :: log_recs())
        -> log_recs().
erts_limits([{Label, Key, Min} | Params], Result) ->
    Rec = report_limit(Label, erlang:system_info(Key), Min),
...
erts_scheduler_rec() ->
    ...
    case erlang:system_info(logical_processors_available) of
        Cores when erlang:is_integer(Cores) ->
            ...
            case (Scheds * 100) of
                Sch when Sch >= SchMin ->
                    {info, "Schedulers: ~b for ~b CPU cores", [Scheds, Cores]};
                _ ->
                    {warning,
                        "Running ~b schedulers for ~b CPU cores,"
                        " at least ~b schedulers (~b%) recommended",
                        [Scheds, Cores, (SchMin div 100), ?ERTS_SCHED_PCT]}
            end;
        _ ->
            {notice,
                "Running ~b schedulers for unknown CPU cores,"
                " at least ~b% recommended", [Scheds, ?ERTS_SCHED_PCT]}
    end.
...
u_limits(ULimitExe) ->
    ...
    OFRec = case OFLimit of
        ...
        _ ->
            report_limit("Open files", OFLimit, ?OS_FILE_MIN)
    end,
    ...
    CFRec = case CFLimit of
        ...
        _ ->
            report_limit("Core dump size", CFLimit, ?OS_CORE_MIN)
    end,
    [OFRec, CFRec].
...
-spec report_limit(
    Label :: nonempty_string(), Val :: integer() | atom(), Min :: pos_integer())
        -> log_rec().
%% @hidden
report_limit(Label, Val, Min) when erlang:is_integer(Val), Val < Min ->
    {warning,
        "~ts limit of ~b is low, at least ~b is recommended",
        [Label, Val, Min]};
report_limit(Label, unknown, Min) ->
    {notice, "~ts limit is unknown, at least ~b is recommended", [Label, Min]};
report_limit(Label, Val, _Min) ->
...
run_exe(Exe, Args) ->
    ...
    ?LOG_DEBUG("Time: ~bms to exec ~ts", [
        erlang:convert_time_unit((erlang:monotonic_time() - Start),
            native, millisecond), lists:join(" ", [Exe | Args])]),
    Result.
